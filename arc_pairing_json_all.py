...
def pairing(seq,ref,res):
    ...
    #print "-------"
    #print wrong
    precision = 0 if len(pairs)==0 else round(len(hit)/2.0/len(pairs),4)
    recall = 0 if len(refpairs)==0 else round(len(hit)/2.0/len(refpairs),4)
    Fscore = 0
    if len(hit) > 0 and precision+recall > 0:
